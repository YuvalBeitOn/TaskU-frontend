<template>
  <section class="board-activiites">
    <side-bar @closeBtn="closeBtn">
      <main class="flex column wrap ">
        <div class="board-activity-title-container">
        ...
        <div class="activity-contianer">
          <activites-board
            v-if="boardActiviites.length"
            :activities="boardActiviites"
          />
        </div>
      </main>
    </side-bar>
...
export default {
  components: { sideBar, activitesBoard },
  computed: {
    boardActiviites() {
      return this.$store.getters.boardActivities
    },
    boardActiviitesTitle() {
      return this.boardActiviites.length
        ? this.boardActiviites.length + ' Activiites on the Board'
        : 'Not have yet activiites'
    }
  },
  methods: {
    closeBtn() {
